...
if __name__ == "__main__":
    ...
    try:
        ...
        # Create the agent
        agent = ChessLLMAgent(args.chess, config=config)
...
#
# class ChessLLMAgent:
#     def __init__(self, chess_script_path: str, config=None, llm_client=None):
#         """
#         Initialize the LLM chess agent.
#
#         Args:
...
#             llm_client: LLM client instance (optional, will be initialized from config if not provided)
#         """
#         self.chess_script_path = chess_script_path
#         self.game_id = None
#         self.process = None
...
#         if llm_client is None and config:
#             self.llm_client = self._init_llm_client_from_config()
#         else:
#             self.llm_client = llm_client
#
#         self.chess_board = None  # Chess board object for easier move generation and FEN access
#
#     def _init_llm_client_from_config(self):
#         """
#         Initialize the LLM client based on the provided configuration.
#
#         Returns:
#             Initialized LLM client or None if configuration is invalid
#         """
#         if not self.config or 'llm' not in self.config:
#             print("Warning: LLM configuration not found in config.")
#             return None
#
...
#
#         if not api_key_env:
#             print("Warning: API key environment variable not specified in config.")
#             return None
#
#         api_key = os.environ.get(api_key_env)
#         if not api_key:
...
#                 )
#             else:
#                 print(f"Warning: Unsupported provider '{provider}' in config.")
#                 return None
#         except Exception as e:
...
#         """
#         if not self.llm_client:
#             raise ValueError("LLM client not initialized. Please provide a valid configuration or client.")
#
#         # Try to get a valid response with retries
#         for attempt in range(max_retries):
#             try:
#                 # Determine how to call the LLM based on provider in config
...
#                     return completion.choices[0].message.content
#                 elif provider == "hf-inference":
#                     # HuggingFace Inference API has a different interface
#                     response = self.llm_client.text_generation(
#                         prompt=prompt,
#                         max_new_tokens=self.config.get('llm', {}).get('max_tokens', 500),
#                         model=self.config.get('llm', {}).get('model_id'),
#                     )
#                     return response
#                 else:
